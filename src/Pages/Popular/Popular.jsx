...
const Popular = () => {
  ...
  <div className="tvShow w-full py-8  px-10 bg-[#141414] min-h-[100vh]">
        <div className="w-full py-10">
         <TitleCards title={"Popular Movies"} category={'popular'} onCardClick={setSelectedCard} />
          <TVSeries tvShowName={'Popular Series'} tvshow_path={'popular'} onItemClick={setSelectedCard} />
          <TitleCards title={"Now Playing Movies"} category={'now_playing'}  onCardClick={setSelectedCard} />
          <TVSeries tvShowName={'Now Playing Series'} tvshow_path={'airing_today'} onItemClick={setSelectedCard} />
        </div>
        <Footer />
        ...
        <div className="modal fixed overflow-y-scroll rounded-md  bg-[#141414] top-1/2 left-1/2 w-3/4 h-2/3 p-5 px-10 -translate-x-1/2  -translate-y-1/2 z-[1000]" style={{  'WebkitOverflowScrolling': 'touch', 'scrollbarWidth': 'none' }}>/
          ...
          <div className="container flex flex-wrap  w-full text-white">
            ...
            <div className="movie-info pb-10 lg:pb-0 relative px-0 md:px-5 w-full lg:w-2/3 ">
              <h1 className='mb-4 text-3xl text-red-600 font-semibold'>{selectedCard.title ?selectedCard.title.toUpperCase() : selectedCard.name.toUpperCase()}</h1>
